...
Layer.prototype.readRange = function(start, end) {
  var results = [];
  ...
  for(var i = start; i < end; i++) {
    var current = i % this.points;
    results.push({
      time: this.buffer.readUInt32LE(this._getPos(current, 0)),
      value: this.buffer.readFloatLE(this._getPos(current, 1))
    });
  }
  return results;
};
...
Layer.prototype.covered = function covered(distance) {
  return distance < this.points * this.precision;
};
